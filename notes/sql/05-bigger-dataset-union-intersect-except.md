# Bigger Dataset, UNION, INTERSECT, EXCEPT in SQL

## What is it?
This note covers working with larger datasets in SQL and introduces set operations such as UNION, INTERSECT, and EXCEPT. It also revisits JOINs, GROUP BY, and aggregate functions to handle more complex queries.
...
The `UNION` and `UNION ALL` operators are used to combine the results of two or more `SELECT` statements. The key difference between them is that `UNION` removes duplicate rows from the result set, while `UNION ALL` includes all rows, including duplicates.
...
The `INTERSECT` and `INTERSECT ALL` operators are used to return the common rows between two `SELECT` statements. The difference is that `INTERSECT` removes duplicates, while `INTERSECT ALL` retains them.
...
The `EXCEPT` and `EXCEPT ALL` operators are used to return rows from the first `SELECT` statement that are not present in the second `SELECT` statement. The difference is that `EXCEPT` removes duplicates, while `EXCEPT ALL` retains them.
...
### Key Points:
- In case of Except, order of operations matters; the first query's results are filtered by the second.